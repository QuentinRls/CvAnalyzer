...
<body>""")
        ...
        if dossier.entete:
            ...
            if dossier.entete.annees_experience:
                ...
                html_parts.append(f'<div class="subtitle">{exp_text}</div>')
        ...
        if dossier.entete.resume_profil:
            html_parts.append(f'<div class="professional-summary">{dossier.entete.resume_profil}</div>')
        ...
        if dossier.diplomes:
            for diplome in dossier.diplomes:
                ...
                if diplome.intitule:
                    html_parts.append(f'🎓 <strong>{diplome.intitule}</strong><br>')
        ...
        if dossier.certifications:
            for cert in dossier.certifications:
                html_parts.append('<div class="certification-item">')
                if cert.intitule:
                    html_parts.append(f'<strong>{cert.intitule}</strong>')
                if cert.organisme:
        ...
        if dossier.experiences_professionnelles:
            ...
            for exp in dossier.experiences_professionnelles:
                ...
                # Construire la durée à partir des dates
                if exp.date_debut or exp.date_fin:
                    duration = f"{exp.date_debut or ''} à {exp.date_fin or ''}".strip()
                    html_parts.append(f'<div class="duration">{duration}</div>')
                html_parts.append('</div>')
                ...
                if exp.environnement_technique:
                    ...
                    # Récupérer toutes les compétences techniques disponibles
                    env_tech = exp.environnement_technique
                    all_techs = []
                    
                    if env_tech.language_framework:
                        all_techs.extend(env_tech.language_framework)
                    if env_tech.outils:
                        all_techs.extend(env_tech.outils)
                    if env_tech.base_de_donnees_big_data:
                        all_techs.extend(env_tech.base_de_donnees_big_data)
                    if env_tech.ci_cd:
                        all_techs.extend(env_tech.ci_cd)
                    if env_tech.tests:
                        all_techs.extend(env_tech.tests)
