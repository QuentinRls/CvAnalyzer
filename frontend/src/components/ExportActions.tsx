...
import { generatePDF, generateGoogleDocs, generatePowerPoint } from '../lib/api';
...
export default function ExportActions({ dossierData, className = '' }: ExportActionsProps) {
  ...
  const handleGenerateGoogleDocs = async () => {
    ...
    try {
      const htmlBlob = await generateGoogleDocs(dossierData);
      ...
      toast.success('Fichier Google Docs généré ! Instructions : 1) Ouvrez le fichier HTML téléchargé dans votre navigateur 2) Sélectionnez tout le contenu (Ctrl+A) 3) Copiez (Ctrl+C) 4) Collez dans un nouveau document Google Docs', {
        duration: 8000
      });
    } catch (error) {
      console.error('Erreur lors de la génération Google Docs:', error);
      toast.error('Erreur lors de la génération Google Docs');
    } finally {
  ...
  return (
    <div className={`flex flex-col sm:flex-row gap-3 ${className}`}>
      ...
      <button
        ...
        className="
          group relative overflow-hidden
          bg-gradient-to-r from-[#4285F4] to-[#1a73e8] 
          hover:from-[#1a73e8] hover:to-[#1567d3]
          text-white font-semibold py-3 px-6 rounded-xl
          transform transition-all duration-200 
          hover:scale-105 hover:shadow-lg
          disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none
      ...
      >
        ...
        {/* Text */}
        <span className="relative z-10">
          {isGeneratingDocs ? 'Génération Docs...' : 'Google Docs'}
        </span>
      </button>
      ...
      >
        ...
        {/* Text */}
        <span className="relative z-10">
          {isGeneratingPPTX ? 'Génération PPTX...' : 'PowerPoint Devoteam'}
        </span>
      </button>
    </div>
  );
